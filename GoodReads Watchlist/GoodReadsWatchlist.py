...
class goodreadswatchlist(MediaAdder):
    ...
    def runShedule(self):
        ...
        for book in books:
            log.debug('ID:%s Title:%s Author:%s Image:%s Rating:%s' % (book['book']['id']['#text'], book['book']['title'], book['book']['authors']['author']['name'], book['book']['image_url'], book['book']['average_rating']))
            additionalData = {}
